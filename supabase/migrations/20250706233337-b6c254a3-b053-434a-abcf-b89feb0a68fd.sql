-- Autonomous AI System Activation - Final Implementation
...
-- Step 2: Add unique constraint to source_category_mapping
ALTER TABLE public.source_category_mapping 
ADD CONSTRAINT IF NOT EXISTS source_category_mapping_unique 
UNIQUE (source_id, category);

-- Step 3: Mass populate source_category_mapping (insert only if not exists)
...
WHERE NOT EXISTS (
  SELECT 1 FROM public.source_category_mapping scm 
  WHERE scm.source_id = gcs.id AND scm.category = category_type
);
...
SELECT 'auction_houses', '["coin auction", "numismatic auction", "collectible auction"]', '["US", "EU", "UK", "AU", "CA"]', 0.8, 5
WHERE NOT EXISTS (SELECT 1 FROM public.source_discovery_config WHERE discovery_type = 'auction_houses')
UNION ALL
SELECT 'dealer_shops', '["coin dealer", "numismatic shop", "bullion dealer"]', '["US", "EU", "UK", "DE", "FR", "IT"]', 0.7, 8
WHERE NOT EXISTS (SELECT 1 FROM public.source_discovery_config WHERE discovery_type = 'dealer_shops')
UNION ALL
SELECT 'marketplace_platforms', '["coin marketplace", "collectible platform", "numismatic exchange"]', '["Global"]', 0.75, 6
WHERE NOT EXISTS (SELECT 1 FROM public.source_discovery_config WHERE discovery_type = 'marketplace_platforms')
UNION ALL
SELECT 'specialized_sources', '["error coin specialist", "banknote expert", "bullion specialist"]', '["US", "EU", "UK", "AU"]', 0.85, 4
WHERE NOT EXISTS (SELECT 1 FROM public.source_discovery_config WHERE discovery_type = 'specialized_sources')
UNION ALL
SELECT 'global_sources', '["collectibles worldwide", "international numismatics", "world coins"]', '["AS", "SA", "AF", "OC"]', 0.7, 10
WHERE NOT EXISTS (SELECT 1 FROM public.source_discovery_config WHERE discovery_type = 'global_sources');
...
SELECT 'coins', 0.15, 0, 0.75
WHERE NOT EXISTS (SELECT 1 FROM public.ai_learning_performance WHERE category = 'coins')
UNION ALL
SELECT 'banknotes', 0.12, 0, 0.70
WHERE NOT EXISTS (SELECT 1 FROM public.ai_learning_performance WHERE category = 'banknotes')
UNION ALL
SELECT 'bullion', 0.10, 0, 0.80
WHERE NOT EXISTS (SELECT 1 FROM public.ai_learning_performance WHERE category = 'bullion')
UNION ALL
SELECT 'error_coins', 0.20, 0, 0.65
WHERE NOT EXISTS (SELECT 1 FROM public.ai_learning_performance WHERE category = 'error_coins')
UNION ALL
SELECT 'error_banknotes', 0.18, 0, 0.60
WHERE NOT EXISTS (SELECT 1 FROM public.ai_learning_performance WHERE category = 'error_banknotes');
...
VALUES ('autonomous_system_activated', '/admin', '{"status": "complete", "mappings": "800+"}', now());