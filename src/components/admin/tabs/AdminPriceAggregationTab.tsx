          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {aggregatedPrices?.length ? 
                Math.round((aggregatedPrices.reduce((sum, p) => sum + (p.confidence_level || 0), 0) / aggregatedPrices.length) * 100) 
                : 0}%
            </div>
            <p className="text-xs text-muted-foreground">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-600">
              {aggregatedPrices?.filter(p => p.price_trend === 'rising').length || 0}
            </div>
            <p className="text-xs text-muted-foreground">
              Coins trending up
            </p>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-red-600">
              {aggregatedPrices?.filter(p => p.price_trend === 'falling').length || 0}
            </div>
            <p className="text-xs text-muted-foreground">
              Coins trending down
            </p>
              ...
              <div key={price.id} className="flex items-center justify-between p-4 border rounded-lg">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {price.grade && (
                      <Badge variant="outline">{price.grade}</Badge>
                    )}
                    <Badge className={getConfidenceColor(price.confidence_level)}>
                      {Math.round((price.confidence_level || 0) * 100)}% confidence
                    </Badge>
                  </div>
                  <div className="text-sm text-muted-foreground">
                    Sample size: {price.sample_size} • Sources: {price.price_sources?.length || 0}
                  </div>
                </div>

                <div className="flex items-center gap-4">
                  <div className="text-right">
                    <div className="text-lg font-bold">${price.current_avg_price?.toFixed(2)}</div>
                    <div className={`text-sm flex items-center gap-1 ${getTrendColor(price.price_trend)}`}>
                      {getTrendIcon(price.price_trend)}
                      {price.trend_percentage ? `${price.trend_percentage.toFixed(1)}%` : '0%'}
                    </div>
                  </div>
                ...
                <div key={history.id} className="flex items-center justify-between p-3 border rounded">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-muted-foreground">
                        {history.external_price_sources?.source_name} • {history.sale_type}
                      </div>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="text-sm">{history.grade}</div>
                    <div className="text-xs text-muted-foreground">
                      {history.sale_date ? new Date(history.sale_date).toLocaleDateString() : 'No date'}
                    </div>
                  </div>
                </div>
              ))}
