...
import { Shield, Database, Globe, Activity } from 'lucide-react';
import { useErrorReferenceSources, useSourcePerformanceMetrics, useVpnProxies } from '@/hooks/admin/useSecurityTables';

const SecurityTablesSection = () => {
  const { data: errorSources = [], isLoading: errorSourcesLoading } = useErrorReferenceSources();
  const { data: performanceMetrics = [], isLoading: performanceLoading } = useSourcePerformanceMetrics();
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Monitor error reference sources, performance metrics, and VPN proxies</CardDescription>
        </CardHeader>
        <CardContent>
          <Tabs defaultValue="error-sources" className="space-y-4">
            ...
            <TabsContent value="error-sources" className="space-y-4">
              {errorSourcesLoading ? (
                <div className="text-center py-8 text-muted-foreground">Loading error reference sources...</div>
              ) : errorSources.length === 0 ? (
                <div className="text-center py-8 text-muted-foreground">No error reference sources found</div>
              ) : (
            ...
            <TabsContent value="performance-metrics" className="space-y-4">
              {performanceLoading ? (
                ...
                <div className="text-center py-8 text-muted-foreground">No performance metrics found</div>
              ) : (
            ...
            <TabsContent value="vpn-proxies" className="space-y-4">
              {vpnLoading ? (
                <div className="text-center py-8 text-muted-foreground">Loading VPN proxies...</div>
              ) : vpnProxies.length === 0 ? (
                <div className="text-center py-8 text-muted-foreground">No VPN proxies found</div>
              ) : (
