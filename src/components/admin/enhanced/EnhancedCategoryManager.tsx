        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {categories?.map((category: Category) => {
              const stats = category.category_usage_stats?.[0];
              return (
                      ...
                      {stats && (
                        <div className="grid grid-cols-2 gap-2 text-xs">
                          <div className="flex items-center gap-1">
                            ...
                            <span>{stats.coins_count} coins</span>
                          </div>
                          <div className="flex items-center gap-1">
                      ...
                      <div className="flex gap-2 pt-2">
                        <Dialog>
                          ...
                          <DialogContent className="max-w-2xl">
                            <DialogHeader>
                              <DialogTitle>{category.name} - Detailed Analytics</DialogTitle>
                            </DialogHeader>
                            {selectedCategory && (
                              <div className="space-y-4">
                                <div className="grid grid-cols-2 gap-4">
                                  <div className="p-4 bg-blue-50 rounded-lg">
                                    ...
                                    <div className="text-sm text-blue-800">Total Coins</div>
                                  </div>
                                ...
                                <div className="space-y-2">
                                  ...
                                  <div className="p-3 bg-gray-50 rounded-lg space-y-1 text-sm">
                                    ...
                                    <p><strong>Display Order:</strong> {selectedCategory.display_order}</p>
                                    <p><strong>Status:</strong> {selectedCategory.is_active ? 'Active' : 'Inactive'}</p>
                                    <p><strong>Last Updated:</strong> {stats?.last_updated ? new Date(stats.last_updated).toLocaleDateString() : 'Never'}</p>
                                  </div>
                                </div>
                              </div>
                            )}
