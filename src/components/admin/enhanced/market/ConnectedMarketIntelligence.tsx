...
const ConnectedMarketIntelligence = () => {
  ...
  const { data: marketAnalytics, isLoading: analyticsLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (error) {
        console.error('❌ Error fetching market analytics for admin:', error);
        throw error;
      }
      
      console.log('✅ Admin market analytics loaded:', data?.length);
      return data || [];
    }
  });
  ...
  const { data: priceSources, isLoading: sourcesLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (error) {
        console.error('❌ Error fetching price sources for admin:', error);
        throw error;
      }
      
      console.log('✅ Admin price sources loaded:', data?.length);
      return data || [];
    }
  });
  ...
  const { data: aggregatedPrices, isLoading: pricesLoading } = useQuery({
    queryKey: ['admin-aggregated-prices'],
    queryFn: async () => {
      const { data, error } = await supabase
      ...
      if (error) {
        console.error('❌ Error fetching aggregated prices for admin:', error);
        throw error;
      }
      
      console.log('✅ Admin aggregated prices loaded:', data?.length);
      return data || [];
    }
  });
  ...
  const { data: analysisResults, isLoading: resultsLoading } = useQuery({
    queryKey: ['admin-market-analysis-results'],
    queryFn: async () => {
      const { data, error } = await supabase
      ...
      if (error) {
        console.error('❌ Error fetching market analysis results for admin:', error);
        throw error;
      }
      
      console.log('✅ Admin market analysis results loaded:', data?.length);
      return data || [];
    }
  });
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle className="flex items-center gap-2">
              ...
              <Badge className="bg-green-100 text-green-800">Full Admin Data Access</Badge>
            </CardTitle>
