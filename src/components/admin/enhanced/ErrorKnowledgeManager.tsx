...
const ErrorKnowledgeManager = () => {
  ...
  const { data: errorKnowledge = [], isLoading } = useQuery({
    queryKey: ['error-knowledge', searchTerm],
    queryFn: async () => {
      let query = supabase
      ...
      if (searchTerm) {
        query = query.or(`error_name.ilike.%${searchTerm}%,error_type.ilike.%${searchTerm}%,description.ilike.%${searchTerm}%`);
      }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-orange-600">{marketData.length}</div>
            <p className="text-xs text-muted-foreground">Pricing records</p>
          </CardContent>
        </Card>
      </div>
