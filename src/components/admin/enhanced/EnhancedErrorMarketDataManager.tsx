...
import { useErrorMarketData } from '@/hooks/useEnhancedErrorKnowledge';

const EnhancedErrorMarketDataManager = () => {
  const [selectedGrade, setSelectedGrade] = useState('all');
  const [selectedRegion, setSelectedRegion] = useState('all');

  const { data: marketData, isLoading } = useErrorMarketData();
              ...
              {filteredData.map((entry) => (
                <TableRow key={entry.id}>
                  <TableCell className="font-medium">
                    {(entry as any).error_coins_knowledge?.error_name || 'Unknown Error'}
                  </TableCell>
                  <TableCell>
...
export default EnhancedErrorMarketDataManager;
