...
const ConnectedErrorDetection = () => {
  ...
  const handleDetectErrors = async (imageUrl: string) => {
    try {
      ...
      const result = await detectErrors.mutateAsync({
        imageHash: 'admin-test-hash-' + Date.now(),
        coinInfo: {
          keywords: ['error', 'coin', 'admin'],
          category: 'admin_test',
      ...
      toast({
        title: "Admin Error Detection Complete",
        description: "AI analysis completed successfully for admin review",
      });
    } catch (error) {
      console.error('Admin error detection failed:', error);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Admin Error Detection System
            <Badge className="bg-red-100 text-red-800">Full Admin Access</Badge>
          </CardTitle>
        </CardHeader>
        <CardContent>
          ...
          <div className="flex gap-4 mb-4">
            ...
            <Button 
              onClick={() => handleDetectErrors('admin-test-image-url')}
              disabled={detectErrors.isPending}
            >
              Test Admin AI Detection
            </Button>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Admin Error Coin Knowledge Base ({filteredErrors.length})</CardTitle>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
