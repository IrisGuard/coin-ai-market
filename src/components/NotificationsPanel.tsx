...
const NotificationItem = ({
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      case 'bid':
        return <Badge variant="outline" className="bg-electric-orange/10 text-electric-orange border-electric-orange/30">New Bid</Badge>;
      case 'outbid':
        return <Badge variant="outline" className="bg-red-500/10 text-red-500 border-red-500/30">Outbid</Badge>;
      case 'auction_end':
        return <Badge variant="outline" className="bg-brand-primary/10 text-brand-primary border-brand-primary/30">Auction Ended</Badge>;
      case 'purchase':
        return <Badge variant="outline" className="bg-electric-emerald/10 text-electric-emerald border-electric-emerald/30">Purchase</Badge>;
      default:
    ...
    <DropdownMenu open={isOpen} onOpenChange={setIsOpen}>
        ...
        <Button variant="ghost" size="icon" className="relative">
          ...
          <AnimatePresence>
            {unreadCount > 0 && (
              ...
              >
                <Badge
                  className="px-1 min-w-[20px] h-5 bg-gradient-to-r from-brand-primary to-electric-orange text-white font-semibold"
                >
                  {unreadCount}
                </Badge>
              </motion.div>
      ...
      <DropdownMenuContent align="end" className="w-96 max-h-[80vh] overflow-auto p-2 shadow-lg backdrop-blur-sm bg-white/80 border border-gray-100">
        <div className="sticky top-0 z-10 flex justify-between items-center py-2 px-1 bg-white/90 backdrop-blur-sm border-b border-gray-100 mb-2">
          <DropdownMenuLabel className="text-lg font-serif bg-gradient-to-r from-brand-primary to-electric-blue bg-clip-text text-transparent">
            Ειδοποιήσεις
          </DropdownMenuLabel>
          {unreadCount > 0 && (
            <Button
          ...
          {isLoading ? (
            <div className="flex justify-center py-12">
              <div className="h-10 w-10 relative">
                ...
                <div className="absolute inset-1 rounded-full border-2 border-t-transparent border-r-electric-orange border-b-transparent border-l-transparent animate-spin animation-delay-150"></div>
                <div className="absolute inset-2 rounded-full border-2 border-t-transparent border-r-transparent border-b-electric-blue border-l-transparent animate-spin animation-delay-300"></div>
              </div>
            </div>
